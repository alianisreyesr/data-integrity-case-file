...
from .database import DB_PATH, get_db, row_to_dict
from .models import (
...
async def list_cases(status: Optional[CASE_STATUSES] = Query(None)):
    async for db in get_db():
        if status:
...
async def create_case(body: CaseCreate, x_actor: str = Header(...)):
    ref = _ref("DI")
    now = _now()
    async for db in get_db():
        await db.execute(
...
async def update_case_status(case_id: int, body: CaseStatusUpdate, x_actor: str = Header(...)):
    async for db in get_db():
        row = await _require_case(db, case_id)
        current = row["status"]
        target = body.status
...
async def close_case(case_id: int, x_actor: str = Header(...)):
    return await update_case_status(case_id, CaseStatusUpdate(status="closed"), x_actor)


@router.get("/cases/{case_id}/alcoa-gaps", response_model=List[AlcoaGapOut])
async def list_gaps(case_id: int):
    async for db in get_db():
...
async def add_gap(case_id: int, body: AlcoaGapCreate, x_actor: str = Header(...)):
    async for db in get_db():
...
async def list_evidence(case_id: int):
    async for db in get_db():
        await _require_case(db, case_id)
        async with db.execute(
...
async def add_evidence(case_id: int, body: EvidenceCreate, x_actor: str = Header(...)):
    async for db in get_db():
        await _require_open_case(db, case_id)
        now = _now()
        await db.execute(
...
async def list_capas(case_id: int):
    async for db in get_db():
        await _require_case(db, case_id)
        async with db.execute(
...
async def add_capa(case_id: int, body: CapaCreate, x_actor: str = Header(...)):
    async for db in get_db():
        await _require_open_case(db, case_id)
        ref = _ref("CAPA")
        now = _now()
...
async def update_capa_status(
    case_id: int, capa_id: int, body: CapaStatusUpdate, x_actor: str = Header(...)
):
    async for db in get_db():
        await _require_open_case(db, case_id)
        async with db.execute(
...
async def audit_log(case_id: Optional[int] = None):
    async for db in get_db():
        if case_id:
            async with db.execute(
...
async def ai_suggest_gaps(case_id: int, x_actor: str = Header(...)):
    async for db in get_db():
...
async def list_ai_suggestions(case_id: int):
    async for db in get_db():
        await _require_case(db, case_id)
        async with db.execute(
...
async def review_ai_suggestion(
    suggestion_id: int, body: AiSuggestionReviewCreate, x_actor: str = Header(...)
):
    async for db in get_db():
        async with db.execute("SELECT * FROM ai_suggestions WHERE id=?", (suggestion_id,)) as cur:
            row = await cur.fetchone()
