"""Per-item, human-editable review records for AI-generated suggestions (async)."""
...
async def list_items(suggestion_id: int):
...
async def review_item(
    suggestion_id: int, item_index: int, body: ItemReviewCreate, x_actor: str = Header(...)
):
    async for db in get_db():
        async with db.execute("SELECT * FROM ai_suggestions WHERE id=?", (suggestion_id,)) as cur:
            suggestion = await cur.fetchone()
